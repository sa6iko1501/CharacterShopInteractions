...
public class Rogue implements Character{
    ...
    public void Rogue(String name, int hp, int mana, int agility, int power, int gold){
        if(name!=null) {
            this.name = name;
        }
        if(hp>0) {
    ...
    @Override
    public void info(){
        System.out.printf("Hi my name is %s, I am a Rogue and my stats are- HP:%d, MANA:%d, AG:%d, POW:%d\n",this.name,this.hp,this.mana,this.agility,this.power);
    }
    @Override
    public void items(){
